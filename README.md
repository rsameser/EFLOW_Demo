...
8. Use Visual Studio Code to deploy the modules using the [deployment.eflow_demo.template.json](./deployment.eflow_demo.template.json)
9. Run the EFLOW+LVA code on the Windows Host
10. Ssh into the EFLOW VM 
11. Run `sudo iptables -A INPUT -p udp --dport 554 -j ACCEPT`
12. Check the EFLOW VM IP `sudo ifconfig`
